...
for (const token of ["lang=\"ar\"", "dir=\"rtl\"", "styles.css", "app.js", "id=\"micButton\"", "id=\"recordingsList\"", "id=\"metronomeOrb\"", "id=\"exportFormat\"", "id=\"bpmValue\"", "id=\"tunerLowLimit\"", "id=\"recordingModeControl\"", "id=\"chromaticSettings\"", "id=\"aboutDialog\"", "id=\"recordingQualityRange\""]) if (!html.includes(token)) failures.push(`index.html missing ${token}`);
...
for (const token of ["--gold-100", ".top-grid", ".bottom-grid", ".note-rosette", ".tuner-bar", ".recording-workflow", ".header-tabs", "@media (max-width: 1200px)", "@media (max-width: 900px)", "@media (max-width: 640px)", "prefers-reduced-motion"]) if (!css.includes(token)) failures.push(`styles.css missing ${token}`);
for (const token of ["getUserMedia", "MediaRecorder", "autocorrelate", "noteFromFrequency", "showToast", "renderRecords", "encodeWav", "encodeMp3", "updateToleranceVisualization", "applySettings", "recordingArmed", "evaluateAutomaticRecording", "beginAutomaticRecording", "buildChromaticSession", "updateChromaticProgress", "recordingQualityThreshold"]) if (!js.includes(token)) failures.push(`app.js missing ${token}`);
